...
export const OrphanedKeysSection: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="text-sm text-gray-500 dark:text-gray-400">Checking for orphaned keys...</div>
    );
  }

  if (keys.length === 0) {
    return (
      <div className="text-sm text-gray-500 dark:text-gray-400">No orphaned deploy keys found.</div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-yellow-400">
        ...
        likely because the app crashed or was force-quit. Please delete them manually from the
        respective service to keep your repositories secure.
      </p>
          ...
          <tbody>
            {keys.map((key) => (
              ...
              >
                ...
                <td className="py-2 pr-4 text-gray-500 dark:text-gray-400 capitalize">
                  {key.service ?? 'github'}
                </td>
