...
DECOMPOSITION (epics only)
    ...
    /workspace/@task-decomposition.json
  {
    ...
    "tasks": [
      { "title": "Scaffold project", "description": "...", "order": 1 },
      { "title": "Add feature A",    "description": "...", "order": 2 },
      { "title": "Add feature B",    "description": "...", "order": 2 }
    ]
  }
  "order" (optional positive integer, default 1): controls dispatch sequence.
  Lower-order tasks are dispatched first; same-order tasks may run in
  parallel. Always assign order 1 to foundation work (scaffolding, DB
  schema, project setup) that later tasks depend on. Omit "order" when
