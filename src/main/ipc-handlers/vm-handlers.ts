...
// All handlers run in the main process and delegate to VMManager and ContainerOrchestrator.
 ...
 * and ContainerOrchestrator methods. VM state changes are broadcast to all renderer
 * windows via VM_STATUS_CHANGED after mutating operations.
 */
export function registerVMHandlers(services: VMServices): void {
  const { vmManager, containerOrchestrator } = services;
  ...
  ipcMain.handle(IPC.VM_STOP, async (_event, projectId: string): Promise<void> => {
    await containerOrchestrator.stopProjectVM(projectId);
    // Broadcast updated VM info (state should now be Stopped)
    try {
      const info = await containerOrchestrator.getProjectVMInfo(projectId);
      if (info) {
        broadcastVMStatus(info);
      }
    } catch (err) {
